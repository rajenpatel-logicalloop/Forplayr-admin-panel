...
// POST: Add new user
mock.onPost('/apps/clubs/add-club').reply(config => {
...
mock.onGet('/api/clubs/list/data').reply(config => {
  ...
  const filteredData = dataToFilter.filter(
    club =>
      (club.companyEmail.toLowerCase().includes(queryLowered) ||
        user.businessName.toLowerCase().includes(queryLowered) ||
...
mock.onDelete('/apps/clubs/delete').reply(config => {
  ...
  const clubId = config.id

  // Convert Id to number
  //clubId = Number(userId)
