...
const ReportInfoCard = ({ selectedReport }) => {
  ...
  const renderReportImg = () => {
    if (
      selectedReport !== null &&
      `https://forplayr.s3.ap-south-1.amazonaws.com/${selectedReport[0]?._id?.editorialData?.coverPage}` !==
        null
    ) {
      return (
        <img
          ...
          width="110"
          alt="report-avatar"
          src={`https://forplayr.s3.ap-south-1.amazonaws.com/${selectedReport[0]?._id?.editorialData?.coverPage}`}
          className="img-fluid rounded mt-3 mb-2"
        />
      );
    } else if (
      selectedReport !== null &&
      `https://forplayr.s3.ap-south-1.amazonaws.com/${selectedReport[0]?._id?.editorialData?.logo}` !==
        null
    ) {
      return (
        <img
          ...
          width="110"
          alt="report-avatar"
          src={`https://forplayr.s3.ap-south-1.amazonaws.com/${selectedReport[0]?._id?.editorialData?.logo}`}
          className="img-fluid rounded mt-3 mb-2"
        />
      );
    } else {
      return (
        <Avatar
          ...
          content={selectedReport[0]?._id?.editorialData?.businessName} 
          contentStyles={{
            borderRadius: 0,
            fontSize: "calc(48px)",
            width: "100%",
          ...
          <div className="info-container">
            {selectedReport[0] !== null ? (
              <ul className="list-unstyled">
                <li className="mb-75">
                  <span className="fw-bolder me-25">{<Title str='Reporteditorial' />} : </span>
                  <span>{selectedReport[0]?._id?.editorialData?.businessName}</span>
                </li>
                <li className="mb-75">
                  <span className="fw-bolder me-25">{<Title str='ReportCount' />} : </span>
                  <span>{selectedReport[0]?.editorialCount}</span>
                </li>
                <li className="mb-75">
                  <span className="fw-bolder me-25">{<Title str='Email' />} : </span>
                  <span>{selectedReport[0]?._id?.editorialData?.companyEmail}</span>
                </li>
                <li className="mb-75">
                  <span className="fw-bolder me-25">{<Title str='PhoneNo' />} : </span>
                  <span>{selectedReport[0]?._id?.editorialData?.companyPhoneNo}</span>
                </li>
