...
const ReportView = () => {
  // ** Store Vars
  const store = useSelector(state => state.reportEditoralList)
  ...
  return store.selectedReport !== null && store.selectedReport !== undefined ? (
    ...
    <Alert color='danger'>
      ...
      <div className='alert-body'>
        {<Title str='Reportwithid' />}: {id} {<Title str='doesnotexist' />}. {<Title str="Check list of all" />} {<Title str='Report' />}: <Link to='/apps/all-reporteditorial-list'>{<Title str='allrerportlist' />}</Link>
      </div>
    </Alert>
  )
}
