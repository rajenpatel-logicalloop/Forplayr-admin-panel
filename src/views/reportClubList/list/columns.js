...
const renderClient = (row) => {
  ...
  if (row?._id?.clubData?.coverPage !== null) {    
    return <Avatar className='me-1' img={`${S3BUCKET}${row?._id?.clubData?.coverPage}`} width='32' height='32' />
  } else if (row?._id?.clubData?.shield !== null) {    
    return <Avatar className='me-1' img={`${S3BUCKET}${row?._id?.clubData?.shield}`} width='32' height='32' />
  } else {
    return (
      <Avatar
        initials
